...
const Jobsteps = ({prevStep, SaveAndExit, nextStep, handleChange, values, moreValues}) => {
  const Continue = e => {
    e.preventDefault();
    nextStep();
  }
      ...
      <form className='Hazard-form'>
        <div className='Hazard-form-content'>
          ...
          <div style={{height: 200}} className='mt-3 overflow-auto'>
            <label className='form-label' htmlFor='jobSteps'>Identify the required steps to complete the job</label>
            <textarea className='form-control' id='jobSteps' rows='4' value={moreValues.jobSteps} onInput={handleChange('jobSteps')}></textarea>
          </div>
